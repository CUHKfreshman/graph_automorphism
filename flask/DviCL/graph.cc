...
Graph* Graph::read_edges(int n, int m, const vector<pair<int,int > >& NodeColor, const vector<pair<int,int> >& Edgelist){
   Graph *g = 0;
  unsigned int nof_vertices;
  unsigned int nof_edges;
  ...
  nof_edges=m;
   ...
   for(int i=0; i<NodeColor.size(); i++){
      vertex=NodeColor[i].first;
      color=NodeColor[i].second;
      g->change_color(vertex, color);
   }

  unsigned int from, to;
  for(int i=0; i<Edgelist.size();i++){
    from=Edgelist[i].first;
    to=Edgelist[i].second;
    g->add_edge(from, to);
  }
