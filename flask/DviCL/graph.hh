...
public:
  ...
  size_t store(int & nof_nodes_s, int & max_level_s){
    nof_nodes_s=nof_nodes;
    max_level_s=max_level;
  }
...
public:
  ...
  static Graph* read_edges(int n, int m, const vector<pair<int,int > >& NodeColor, const vector<pair<int,int> >& Edgelist);

  //static Graph* read_graph(char const* filename);
  /**
   * Write the graph to a file in a variant of the DIMACS format.
   * See the <A href="http://www.tcs.hut.fi/Software/bliss/">bliss website</A>
   * for the definition of the file format.
