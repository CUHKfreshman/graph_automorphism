...
def Interaction(Graph,i,j):
    sigi = IC(Graph,i,0.2,20)
    sigj = IC(Graph,[j],0.2,20)
    tmp = i.copy()
    tmp.append(j)
    sigij = IC(Graph,tmp,0.2,20)
    return sigij - sigi

def E(Graph,vs,i):
    sigall = IC(Graph,vs,0.2,20,0)
    sigi = IC(Graph,[i],0.2,20,0)
    tmp = vs.copy()
    tmp.remove(i)
    sigall_i = IC(Graph,tmp,0.2,20,0)
    return sigall - sigall_i - sigi,sigall - sigall_i
def enumerate_all(nodenum, AutoTree,  Graph):

    vs = [107,3437,0,686,348,1684,1912,3980,698,21]

    #vs =[0,1,2,3,4,5]
    #print(IC(Graph,vs, 0.2, 100))
#    while 
#    for i in vs:
#        print(E(Graph,vs,i))
#    max_i = 0
...
#    cand = -1
#    for i in vs:
#        if (IC(Graph,[i],0.2,10)>max_i):
#            cand = [i]
#            max_i = IC(Graph,[i],0.2,10)
#        #print(cand)
#
#   # print(s/9)
#    marginal = 0
    ...
    print(IC(Graph,vv, 0.2, 1,1))
    print(IC(Graph,vs, 0.2, 1,1))
    while 1:
        profit = IC(Graph,vs, 0.2, 100,0)
        m = 99999
        tmp_v = -1
        #print(cand)
        in_sum = 0
...
def ssm_generator(filename, raw_seed_list):
    ...
    AutoTree, SubTree, max_depth = read_AT("./facebookat.txt")
...
if __name__ == "__main__":
    result = ssm_generator('./facebook1.txt',[])
    print(result)
