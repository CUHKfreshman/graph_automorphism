...
def ssm_wrapper(v, nodenum, AutoTree, Graph):
    return (v, ssm(0, [v], get_is_seed([v], nodenum), AutoTree, Graph))

def ssm_wrapper_helper(args):
    return ssm_wrapper(*args)

def enumerate_all(nodenum, AutoTree, Graph):
    with Pool() as pool:
        results = pool.map(ssm_wrapper_helper, [(v, nodenum, AutoTree, Graph) for v in AutoTree[0].vertex_list])
    return dict(results)
...
#main()
def enumerate_all_old(nodenum, AutoTree,  Graph):
